...
serve(async (req: Request) => {
  ...
  // Calculate the date 48 hours ago from the current time
  const fortyEightHoursAgo = new Date(Date.now() - 48 * 60 * 60 * 1000).toISOString();
  console.log("cutoff time", fortyEightHoursAgo);
  ...
  const { data, error } = await supabase
    ...
    `)
    .gte("SourceArticle.created_at", fortyEightHoursAgo); // Filter for articles created within the last 48 hours
