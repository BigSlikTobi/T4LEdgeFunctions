...
interface FranchiseViewResponse {
  headline: string;
  content: string;
  team: string;
  language: string;
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Get franchise view data
    console.log('Fetching franchise view for cluster_id:', cluster_id);
    const { data: franchiseViewData, error: franchiseViewError } = await supabaseClient
      .from("cluster_franchise_view")
      .select("id, headline, content, team")
      .eq("cluster_id", cluster_id)
      .single();

    if (franchiseViewError) {
      console.error("Error fetching franchise view:", franchiseViewError);
      return new Response(
        JSON.stringify({ error: "Error fetching franchise view" }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json; charset=utf-8" }
        }
    ...
    if (!franchiseViewData) {
      return new Response(
        JSON.stringify({ error: "Franchise view not found" }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json; charset=utf-8" }
        }
    ...
    const response: FranchiseViewResponse = {
      headline: franchiseViewData.headline,
      content: franchiseViewData.content,
      team: franchiseViewData.team,
      language: 'en'  // Default language initially
    };

    // Get translations from cluster_franchise_view_int table
    console.log('Fetching translations for franchise view ID:', franchiseViewData.id);
    const { data: translationData, error: translationError } = await supabaseClient
      .from("cluster_franchise_view_int")
      .select("language_code, headline, content")
      .eq("cluster_franchise_view_id", franchiseViewData.id);
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/franchise_view_by_id' \
  ...
  curl -i --location --request GET 'http://127.0.0.1:54321/functions/v1/franchise_view_by_id?cluster_id=your-cluster-uuid-here' \
    --header 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0'
*/
