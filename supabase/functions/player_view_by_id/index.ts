...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    console.log('Fetching player view for cluster_id:', cluster_id);
    const { data: playerViewData, error: playerViewError } = await supabaseClient
      .from("cluster_player_view")
      .select("id, headline, content, player")
      .eq("cluster_id", cluster_id)
      .single();

    if (playerViewError) {
      console.error("Error fetching player view:", playerViewError);
      return new Response(
        JSON.stringify({ error: "Error fetching player view" }),
        { 
          status: 500, 
          headers: { ...corsHeaders, "Content-Type": "application/json; charset=utf-8" }
        }
    ...
    if (!playerViewData) {
      return new Response(
        JSON.stringify({ error: "Player view not found" }),
        {
          status: 404,
          headers: { ...corsHeaders, "Content-Type": "application/json; charset=utf-8" }
        }
    ...
    const response: PlayerViewResponse = {
      headline: playerViewData.headline,
      content: playerViewData.content,
      player: playerViewData.player,
      language: 'en'  // Default language initially
    };

    // Get translations from cluster_player_view_int table
    console.log('Fetching translations for player view ID:', playerViewData.id);
    const { data: translationData, error: translationError } = await supabaseClient
      .from("cluster_player_view_int")
      .select("language_code, headline, content")
      .eq("cluster_player_view_id", playerViewData.id);
...
/* To invoke locally:
  ...
  curl -i --location --request POST 'http://127.0.0.1:54321/functions/v1/player_view_by_id' \
  ...
  curl -i --location --request GET 'http://127.0.0.1:54321/functions/v1/player_view_by_id?cluster_id=your-cluster-uuid-here' \
    --header 'Authorization: Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0'
*/
